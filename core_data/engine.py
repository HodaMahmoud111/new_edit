...
BROAD_EXERCISE_CORE_PATTERN = re.compile(
    r"\b(?:combined\s+)?hip-\s+and\s+knee-targeted\s+exercises?\b",
    re.I,
)
EXERCISE_SPECIALISATION_PATTERN = re.compile(
    r"\b(?:blood[-\s]?flow\s+restriction|bfr|biofeedback|gait\s+retraining|"
    r"forefoot[-\s]?strike|high[-\s]?repetition|resisted\s+knee\s+extension)\b",
    re.I,
)
...
class RagEngine:
    ...
    def clinical_retrieval(self, question: str, k: int = TOP_K) -> tuple[list[tuple[dict[str, Any], float]], str]:
        ...
        # A broad Exercise Therapy question has one primary CPG recommendation.
        # Add that exact PDF-native passage before the generic recommendation
        # index so tie ordering cannot let a restricted intervention dominate
        # Gemini's context.
        direct_support = [
            *self._core_exercise_support(question),
            *self._corpus_direct_guideline_support(question),
        ]
    ...
    @staticmethod
    def _is_broad_exercise_request(question: str) -> bool:
        terms = RagEngine._guideline_question_terms(question)
        return (
            terms == ("exercise",)
            and not EXERCISE_SPECIALISATION_PATTERN.search(normalize_for_retrieval(question))
        )

    @staticmethod
    def _is_core_exercise_recommendation(text: str) -> bool:
        """Recognize the CPG's primary exercise statement despite PDF line breaks."""
        source = normalize_for_retrieval(text)
        return (
            "clinicians should include exercise therapy" in source
            and "hip" in source
            and "knee-targeted" in source
        )

    def _core_exercise_support(self, question: str) -> list[tuple[dict[str, Any], float]]:
        if not self._is_broad_exercise_request(question):
            return []
        return [
            (chunk, 5.0)
            for chunk in self.chunks
            if self._is_core_exercise_recommendation(chunk["original_text"])
        ]
    ...
    def _corpus_direct_guideline_support(self, question: str) -> list[tuple[dict[str, Any], float]]:
        ...
        normalized_question = normalize_for_retrieval(question)
        is_broad_exercise_request = self._is_broad_exercise_request(question)
        for chunk in index:
            ...
            # The PFP CPG names combined hip- and knee-targeted exercise as the
            # primary intervention. Pin that document-native recommendation to
            # the leading context position only for an unqualified exercise
            # question. Narrow questions such as BFR retain their own evidence.
            if is_broad_exercise_request:
                if BROAD_EXERCISE_CORE_PATTERN.search(source):
                    score += 3.0
                elif EXERCISE_SPECIALISATION_PATTERN.search(source):
                    score -= 0.50
